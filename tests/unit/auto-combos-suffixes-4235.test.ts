...
test("every pack carries quality>0 and reliability>0", () => {
  for (const [name, w] of Object.entries(modePacks.MODE_PACKS)) {
    assert.ok(Number((w as any).quality) > 0, `${name} quality>0`);
    assert.ok(Number((w as any).reliability) > 0, `${name} reliability>0`);
    const sum = Object.values(w as any).reduce((a: number, b: any) => a + Number(b), 0);
