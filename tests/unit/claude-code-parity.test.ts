...
import { getClaudeCodeCompatibleVersion } from "../../open-sse/services/claudeCodeCompatible.ts";
...
describe("signRequestBody", () => {
  it("replaces cch=00000 placeholder with computed hash", async () => {
    const body = `{"x-anthropic-billing-header":"cc_version=${getClaudeCodeCompatibleVersion()}.abc; cch=00000;","model":"claude"}`;
...
describe("computeFingerprint", () => {
  it("returns a 3-character hex string", () => {
    const fp = computeFingerprint("Hello, world!", getClaudeCodeCompatibleVersion());
  ...
  it("is deterministic — same text + version always produces the same fingerprint", () => {
    const fp1 = computeFingerprint("Hello, world!", getClaudeCodeCompatibleVersion());
    const fp2 = computeFingerprint("Hello, world!", getClaudeCodeCompatibleVersion());
    assert.equal(fp1, fp2, "fingerprint must be deterministic");
  });

  it("changes when fingerprint version changes", () => {
    const fp1 = computeFingerprint("same text", getClaudeCodeCompatibleVersion());
  ...
  it("handles short messages safely — uses '0' for missing indices", () => {
    ...
    const fp = computeFingerprint("hi", getClaudeCodeCompatibleVersion());
    assert.ok(fp.length === 3, "short input should not throw and return 3-char fingerprint");
  });

  it("handles empty string without throwing", () => {
    const fp = computeFingerprint("", getClaudeCodeCompatibleVersion());
