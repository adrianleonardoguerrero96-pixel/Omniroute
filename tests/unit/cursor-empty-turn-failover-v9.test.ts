...
const { newStreamCtx, processFrame, ctxProducedSignal } = await import(
  "../../open-sse/executors/cursor"
);
...
function makeFabricatedEmptyCursorResponse(): Response {
  const chunks = [
    JSON.stringify({
...
test("v9 chain: healthy cursor stream with real content still passes validation", async () => {
  const chunks = [
    JSON.stringify({
      ...
      object: "chat.completion.chunk",
      model: "cursor/cursor-grok-4.6-high",
      choices: [{ index: 0, delta: { role: "assistant", content: "Working on it" }, finish_reason: null }],
    }),
    JSON.stringify({
      id: "chatcmpl-cursor-healthy",
      object: "chat.completion.chunk",
