...
async function buildUnifiedModelsResponseCore(
  ...
  try {
    ...
    const shouldHidePaid = (
      ...
      if (isFreeForProvider(provider, { id: modelId, pricing: pricing as any, isFree })) return false;
      return true;
    };
      ...
      for (const [providerId, rawProviderCustomModels] of Object.entries(customModelsMap)) {
        ...
        for (const model of providerCustomModels) {
          ...
          if ((model as any)?.isFree === true) {
            if (!isTrustedCustomFree(canonicalProviderId, { id: modelId, pricing: (model as any).pricing, isFree: true } as any)) continue;
          } else if (
            shouldHidePaid(
              canonicalProviderId,
              modelId,
