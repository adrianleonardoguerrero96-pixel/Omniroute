...
export class CursorService {
  ...
  async validateImportToken(accessToken: string, machineId?: string) {
    ...
    return {
      ...
      // Derived from the token's own JWT `exp` claim (weeks, not hours).
      // Hardcoding 24h here produced a false "Token expired" on the connection
      // test roughly a day after import — see resolveCursorTokenTtlSeconds.
      expiresIn: resolveCursorTokenTtlSeconds(accessToken),
